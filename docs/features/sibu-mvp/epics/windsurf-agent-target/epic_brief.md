...
## Summary
Add Windsurf as a first-class Sibu agent target while preserving Sibu's cross-agent workflow model and avoiding deep editor automation.
...
## User Stories
- [Use Windsurf in the Sibu workflow loop](./stories/01-use-windsurf-in-the-sibu-workflow-loop.md)

## Acceptance Criteria
- Windsurf is available as a supported agent target alongside Codex, Claude, and Gemini.
- A Windsurf-selected repo uses the same common `AGENTS.md` and shared skill files expected by Sibu.
- `sibu doctor` and `sibu sync` treat Windsurf state as supported.
