# Use Windsurf in the Sibu Workflow Loop

## Epic
[Windsurf Agent Target](../epic_brief.md)

## User Story
As a developer who uses Windsurf, I want Sibu to support Windsurf as an agent target, so that I can participate in the same managed AI workflow loop as Codex, Claude, and Gemini users.
...
- Changing the common Sibu skill layout away from `.agents/skills/`.

## Acceptance Criteria
- A fresh repo can select Codex, Claude, Gemini, and Windsurf during `sibu init`.
- A Windsurf-selected repo records `windsurf` as a supported selected agent.
- `sibu doctor` does not reject a valid state file because it includes Windsurf.
- `sibu sync` handles Windsurf-selected workflow targets without expecting an unnecessary Windsurf-specific file.
