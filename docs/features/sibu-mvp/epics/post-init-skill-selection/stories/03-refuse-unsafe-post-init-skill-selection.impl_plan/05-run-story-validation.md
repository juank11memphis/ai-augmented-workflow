...
- Manually smoke test a dirty Sibu repo by modifying a managed file, running `sibu skills use typescript`, and confirming no files change and the output points to `sibu sync`.
...
- The dirty-repo smoke test confirms `sibu skills use typescript` exits without file or state changes and clearly points to `sibu sync`.
