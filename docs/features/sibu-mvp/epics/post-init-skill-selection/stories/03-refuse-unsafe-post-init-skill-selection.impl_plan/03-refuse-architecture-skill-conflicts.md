...
Ensure `sibu skills use <skill_name>` fails safely when a different architecture skill is already selected, preserving the existing architecture choice and workflow files.
...
- Add handler-level coverage proving architecture conflicts do not create files, refresh `AGENTS.md`, or update `.sibu/state.json`.
...
- The output avoids implying that Sibu silently replaced or removed the existing architecture skill.
