...
Make the selected-skill application path prove every touched file is safe before creating the skill file, refreshing `AGENTS.md`, or updating `.echo/state.json`.
...
- Return concise failure output that points to `echo sync` for workflow drift cleanup.
...
- For a clean repo with an unrecorded existing `.agents/skills/typescript/SKILL.md`, `echo skills use typescript` exits non-zero and leaves all files unchanged.
- For a repo where `AGENTS.md` changed since the recorded hash, `echo skills use typescript` exits non-zero and leaves all files unchanged.
- The handler computes the full application plan and runs preflight before the first write operation.
- Failure output clearly says the command cannot safely proceed and tells the user to run `echo sync` when cleanup is needed.
