...
- Manually smoke test a dirty Echo repo by modifying a managed file, running `echo skills use typescript`, and confirming no files change and the output points to `echo sync`.
...
- The dirty-repo smoke test confirms `echo skills use typescript` exits without file or state changes and clearly points to `echo sync`.
