...
## Goal
Validate the completed `echo skills use <skill_name>` clean-state path against the story acceptance criteria and capture any remaining risk before marking the story complete.

## Scope

- Run the automated validation commands required by the story.
- Smoke test `echo skills use typescript` in an existing clean Echo-enabled repo or a temporary initialized repo that represents one.
- Smoke test the already-selected case and confirm it exits successfully without changing files.
- Inspect resulting `AGENTS.md` and `.echo/state.json` only enough to confirm the new routing and managed metadata are present.
...
- docs/features/echo-mvp/epics/post-init-skill-selection/stories/02-add-a-selectable-skill-when-workflow-state-is-clean.md
...
- A clean-repo smoke test of `echo skills use typescript` creates and records the TypeScript skill.
- `AGENTS.md` includes the newly selected TypeScript routing instruction after the smoke test.
- `.echo/state.json` reflects the new TypeScript selection and managed file metadata after the smoke test.
- Re-running `echo skills use typescript` succeeds as a no-op and does not change files.
