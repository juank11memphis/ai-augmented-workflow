...
## Goal
Create a reusable preflight helper that lets `echo skills use` verify the current Echo workflow is clean before any file or state mutation happens.
...
- Treat missing or invalid `.echo/state.json`, actionable sync previews, missing managed files, local edits, unreviewed template updates, unknown templates, and unrecorded expected files as not clean.
- Return a small explicit result that the handler can convert into concise user-facing output that points to `echo sync` when cleanup is needed.
- Preserve `echo doctor` and `echo sync` behavior while extracting only the reusable logic needed by this command.
- Do not write files, update `.echo/state.json`, or select a skill in this step.
...
- Dirty or invalid workflow states produce a failure result with enough context to tell the user to run `echo sync` first.
