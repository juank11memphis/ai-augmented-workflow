...
- If `echo skills use` is not implemented yet, record that the `echo skills use nope` smoke test is blocked by the later command-handler story rather than implementing the command here.
...
- src/shared/types.ts
- docs/features/echo-mvp/epics/post-init-skill-selection/stories/01-resolve-a-selectable-skill-by-name.md
...
- Any inability to run `echo skills use nope` is explicitly noted as blocked by the not-yet-implemented command handler, not silently ignored.
