...
- Smoke test a fresh repo selecting Codex, Claude, Gemini, and Windsurf through `echo init` or an equivalent scripted initialization using the same catalog and target/state writers when interactive automation is impractical.
- Smoke test `echo doctor` against the Windsurf-selected repo and confirm it does not reject `windsurf`.
- Smoke test `echo sync` against the Windsurf-selected repo and confirm it does not expect a Windsurf-specific support file.
...
- `pnpm check` passes.
- `pnpm test` passes.
- The fresh-repo Windsurf smoke test records `windsurf` in `.echo/state.json` with Codex, Claude, and Gemini.
- `echo doctor` passes for the Windsurf-selected repo.
- `echo sync` does not report a missing Windsurf-specific file.
