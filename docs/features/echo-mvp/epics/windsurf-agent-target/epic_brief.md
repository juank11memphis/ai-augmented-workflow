...
## Summary
Add Windsurf as a first-class Echo agent target while preserving Echo's cross-agent workflow model and avoiding deep editor automation.
...
## User Stories
- [Use Windsurf in the Echo workflow loop](./stories/01-use-windsurf-in-the-echo-workflow-loop.md)

## Acceptance Criteria
- Windsurf is available as a supported agent target alongside Codex, Claude, and Gemini.
- A Windsurf-selected repo uses the same common `AGENTS.md` and shared skill files expected by Echo.
- `echo doctor` and `echo sync` treat Windsurf state as supported.
