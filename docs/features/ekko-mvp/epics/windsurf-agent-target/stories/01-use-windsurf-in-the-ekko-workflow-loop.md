...
- Changing the common Ekko skill layout away from `.agents/skills/`.

## Acceptance Criteria
- A fresh repo can select Codex, Claude, Gemini, and Windsurf during `ekko init`.
- A Windsurf-selected repo records `windsurf` as a supported selected agent.
- `ekko doctor` does not reject a valid state file because it includes Windsurf.
- `ekko sync` handles Windsurf-selected workflow targets without expecting an unnecessary Windsurf-specific file.
