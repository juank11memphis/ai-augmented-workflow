...
## Goal
Add the `ekko skills use <skill_name>` command contract, CLI registration, and dispatch wiring so the command reaches a dedicated feature handler without putting business logic in the CLI entrypoint.
...
- Register `ekko skills use <skill_name>` under the existing `skills` command in the CLI program.
...
- `ekko skills use <skill_name>` is registered by Commander and dispatches a `skills:use` command object.
