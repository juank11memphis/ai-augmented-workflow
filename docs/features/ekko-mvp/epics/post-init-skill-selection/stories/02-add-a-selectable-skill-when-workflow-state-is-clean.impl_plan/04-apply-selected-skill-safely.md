...
- Update `.ekko/state.json` through existing state-writing logic so selected skill metadata, managed file hashes, and template versions are current.
...
- In a clean Ekko repo, `ekko skills use typescript` creates `.agents/skills/typescript/SKILL.md` from `templates/skills/typescript/SKILL.md` when TypeScript is available and not already selected.
- `AGENTS.md` includes the TypeScript routing instruction after the command completes.
- `.ekko/state.json` includes `typescript` in `selectedLanguageSkills` and records current managed metadata for the new skill file and refreshed `AGENTS.md`.
