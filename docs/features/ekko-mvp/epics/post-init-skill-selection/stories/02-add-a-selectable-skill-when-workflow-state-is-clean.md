...
## Context
The feature brief requires explicit post-init skill selection. The technical design says the command should create the selected skill's `.agents/skills/<skill>/SKILL.md`, refresh `AGENTS.md` routing, and update `.ekko/state.json` only after clean-state validation.

## Scope
- Register and dispatch `ekko skills use <skill_name>`.
...
- Update `.ekko/state.json` with selected skill metadata and current hashes/template versions.
...
## Acceptance Criteria
- In a clean Ekko repo, `ekko skills use typescript` creates and records the TypeScript skill when it is available and not already selected.
- `AGENTS.md` routing includes the newly selected skill after the command completes.
- `.ekko/state.json` reflects the new selection and managed file metadata.
...
- Smoke test an existing clean repo with `ekko skills use typescript`.
