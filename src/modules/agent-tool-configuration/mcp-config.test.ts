...
describe('resolveMcpConfigAgentId', () => {
  it('resolves supported MCP config template paths', () => {
    assert.equal(resolveMcpConfigAgentId('.codex/config.toml'), 'codex');
    assert.equal(resolveMcpConfigAgentId('mcp/claude/.mcp.json'), 'claude');
    assert.equal(resolveMcpConfigAgentId('mcp/gemini/settings.json'), 'gemini');
    assert.equal(resolveMcpConfigAgentId('.gemini/settings.json'), 'gemini');
    assert.equal(resolveMcpConfigAgentId('AGENTS.md'), undefined);
  });
});

describe('JSON MCP config parsing', () => {
  it('preserves existing JSON object properties when rendering MCP servers', () => {
  ...
  it('throws when base JSON content is not an object', () => {
    assert.throws(
      () => renderMcpConfig({ agentId: 'claude', baseContents: '[]', selectedMcpServers: selectedGithubMcpServers }),
      /Expected JSON template to contain an object\./,
    );
  });
});
