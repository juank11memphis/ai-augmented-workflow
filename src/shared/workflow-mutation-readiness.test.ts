...
import { getWorkflowTargets, renderMissingWorkflowFiles, writeEchoState } from './workflow-targets.js';
...
describe('getWorkflowMutationReadiness', () => {
  it('is not ready when state is missing', () => {
    const rootPath = fs.mkdtempSync(path.join(os.tmpdir(), 'echo-readiness-missing-'));
    temporaryRoots.push(rootPath);

    assert.deepEqual(getWorkflowMutationReadiness({ rootPath, statePath: path.join(rootPath, '.echo/state.json') }), {
      ok: false,
      message: '.echo/state.json is missing.',
      hint: 'Run `echo init` before selecting project skills.',
    });
  });

  it('is not ready when state cannot be parsed', () => {
    const rootPath = fs.mkdtempSync(path.join(os.tmpdir(), 'echo-readiness-unparseable-'));
    temporaryRoots.push(rootPath);
    const statePath = path.join(rootPath, '.echo/state.json');
    fs.mkdirSync(path.dirname(statePath), { recursive: true });
    fs.writeFileSync(statePath, '{not json', 'utf8');
    const beforeSnapshot = snapshotFiles(rootPath, ['.echo/state.json']);

    assert.deepEqual(getWorkflowMutationReadiness({ rootPath, statePath }), {
      ok: false,
      message: '.echo/state.json could not be parsed.',
      hint: 'Run `echo init` before selecting project skills.',
    });
    assert.deepEqual(snapshotFiles(rootPath, ['.echo/state.json']), beforeSnapshot);
  });

  it('is not ready when state is structurally invalid', () => {
    const rootPath = fs.mkdtempSync(path.join(os.tmpdir(), 'echo-readiness-invalid-'));
    temporaryRoots.push(rootPath);
    const statePath = path.join(rootPath, '.echo/state.json');
    fs.mkdirSync(path.dirname(statePath), { recursive: true });
    ...
    assert.deepEqual(getWorkflowMutationReadiness({ rootPath, statePath }), {
      ok: false,
      message: '.echo/state.json is not a valid Echo state file.',
      hint: 'Run `echo init` before selecting project skills.',
    });
    assert.deepEqual(snapshotFiles(rootPath, ['.echo/state.json']), beforeSnapshot);
  });
  ...
  it('is ready for a clean initialized repo', () => {
    ...
    const result = getWorkflowMutationReadiness({ rootPath, statePath: path.join(rootPath, '.echo/state.json') });
    ...
    assert.equal(result.manifest.templateVersion, '41');
  ...
  it('is not ready when a managed file has local edits', () => {
    ...
    const beforeSnapshot = snapshotFiles(rootPath, ['AGENTS.md', '.echo/state.json']);
    ...
    assert.equal(result.hint, 'Run `echo sync` to review workflow drift before selecting a skill.');
    assert.equal(result.actionablePreviews?.some((preview) => preview.relativePath === 'AGENTS.md' && preview.status === 'modified'), true);
    assert.deepEqual(snapshotFiles(rootPath, ['AGENTS.md', '.echo/state.json']), beforeSnapshot);
  });
});
...
function createCleanInitializedRepo({
  ...
  const rootPath = fs.mkdtempSync(path.join(os.tmpdir(), 'echo-readiness-clean-'));
  temporaryRoots.push(rootPath);
