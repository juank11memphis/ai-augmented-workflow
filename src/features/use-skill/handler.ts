...
import { getSelectedAgentsFromState, getWorkflowTargets, renderMissingWorkflowFiles, writeEkkoState } from '../../shared/workflow-targets.js';
...
function applySelectedSkill({
  ...
  if (preflightError) {
    ...
    log.info('Run `ekko sync` to review workflow drift before selecting a skill.');
    process.exitCode = 1;
    return;
  }
...
function getSkillApplicationPreflightError({ rootPath, state, plan }: { rootPath: string; state: EkkoState; plan: SkillApplicationPlan }): string | undefined {
  ...
  if (!agentsManagedFile) {
    return 'AGENTS.md is not recorded in `.ekko/state.json`.';
  }
  ...
  if (agentsCurrentHash !== agentsManagedFile.sha256) {
    return 'AGENTS.md has changed since Ekko last recorded it.';
  }
...
function getSelectedFrameworkSkillsFromState(state: EkkoState): SelectableFrameworkSkill[] {
