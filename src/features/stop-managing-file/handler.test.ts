...
describe('stopSelectedSkill', () => {
  it('stops a selected framework skill by selectable skill id', () => {
    ...
    const state = JSON.parse(fs.readFileSync(path.join(rootPath, '.ekko/state.json'), 'utf8'));
  ...
  it('rejects raw managed file paths', () => {
    ...
    assert.equal(result.message, 'Unknown skill `.agents/skills/nextjs/SKILL.md`. Run `ekko skills list` to see available skills.');
  });
...
function createInitializedRepoWithNextjs(): string {
  const rootPath = fs.mkdtempSync(path.join(os.tmpdir(), 'ekko-stop-skill-test-'));
