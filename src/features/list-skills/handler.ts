...
export async function handleListSkills(_command: ListSkillsCommand): Promise<void> {
  ...
  if (!stateResult.ok) {
    ...
    log.info('Run `ekko init` before selecting project skills.');
  }
...
function getLanguageSkillItems(state: EkkoState | undefined): SkillListItem[] {
...
function getFrameworkSkillItems(state: EkkoState | undefined): SkillListItem[] {
...
function getArchitectureSkillItems(state: EkkoState | undefined): SkillListItem[] {
